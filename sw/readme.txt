Readme for COE3DQ5 Project Software Model 2021
...
Each of these files compile independently. The development has been done under Linux. 
If you are a Windows user, the safest thing is to install Cygwin and run GCC. 

To compile with GCC: gcc source_name.c -lm -o executable_name

It is also worth noting that the C model has been written in ANSI C. 
Therefore it should compile and run on other platforms.
...
This step is optional if you wish to produce new PPM files from BMP files.
It converts a .bmp file (Windows bitmap) to a .ppm file. The data in a .ppm file
is organized as R0, G0, B0, R1, G1, B2, etc. and the pixel order goes across first,
then down (like lab 5 experiment 1).

encode_all (mandatory)

This step is mandatory to run in software in order to produce the debug files! 
It encodes the .ppm file to a .mic15 file (McMaster Image Compression, revision 15). 
The details of encoding are provided in the project document. As a summary, first 
we do color space conversion, then downsampling of U and V, then IDCT on 8x8 blocks, 
and finally lossless decoding and quantization.

decode_m3 (mandatory)

This is a reference for milestone 3 in hardware. The .mic15 file is lossless decoded
...
PSNR between two ppm images (see project document for further details).
...
used to read from the SRAM and display on the VGA monitor. This backbone from
lab 5 experiment 4 is given to you already. 

In your work, the first hardware you will design is that which processes data
...
Milestone 3 is the first stage of decoding and last one to be implemented.

Extra note:

The files produced by the software model (.sram_d0, .sram_d1, ...) should be placed 
in the data sub-folder.
